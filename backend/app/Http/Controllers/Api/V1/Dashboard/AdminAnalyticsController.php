...
class AdminAnalyticsController extends Controller
{
    public function __invoke(): JsonResponse
    {
        $data = Cache::remember('admin_analytics', 60, function () {
            ...
            $totalChunks = ContentChunk::count();
            ...
            $booksThisWeek = Book::where('created_at', '>=', now()->subWeek())->count();
            $chunksSinceYesterday = ContentChunk::where('created_at', '>=', now()->subDay())->count();
            ...
            $queries = ChatMessage::where('created_at', '>=', $startDate)
                ->where('role', 'user')
            ...
            // Fill Quizzes
            $quizzes = QuizAttempt::where('created_at', '>=', $startDate)
                ->selectRaw('DATE(created_at) as date, count(*) as count')
                ->groupBy('date')
                ->pluck('count', 'date');
            ...
            $activeUsers = DB::table('chat_sessions')
