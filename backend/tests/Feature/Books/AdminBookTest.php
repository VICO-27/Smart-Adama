...
it('admin can upload a manuscript with the book', function () {
    Storage::fake('local');
    Queue::fake([ProcessUploadedPdfJob::class]);
    ...
    Queue::assertPushed(ProcessUploadedPdfJob::class, fn ($job) => $job->book->id === $book->id);
});
...
it('publish enqueues the ingestion job without blocking', function () {
    ...
    Queue::assertPushed(IngestChapterJob::class, fn ($job) => $job->chapterId === $chapter->id
    );
});
