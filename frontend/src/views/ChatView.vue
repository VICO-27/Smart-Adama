<template>
  ...
  >
    ...
    >
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-6" ref="messagesContainerRef">
        ...
        <!-- Streaming: thinking dots when waiting, text when tokens arrive -->
          ...
          <div class="bg-(--rt-surface) border border-(--rt-border) p-4 rounded-2xl rounded-tl-sm text-sm text-(--rt-text) shadow-sm leading-relaxed">
            ...
            <template v-else>
              <div class="flex items-center gap-1.5 py-1">
                <span class="w-2 h-2 rounded-full bg-(--rt-muted) animate-bounce" style="animation-delay:0ms"></span>
                <span class="w-2 h-2 rounded-full bg-(--rt-muted) animate-bounce" style="animation-delay:150ms"></span>
                <span class="w-2 h-2 rounded-full bg-(--rt-muted) animate-bounce" style="animation-delay:300ms"></span>
              </div>
            </template>
          </div>
        </div>

        <!-- Error banner when AI call fails -->
        <div v-if="chatStore.streamError && !chatStore.streaming" class="flex flex-col w-full max-w-[95%]">
          <span class="text-xs font-bold text-(--rt-text) mb-1 ml-1 flex items-center gap-1">
            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
            Smart Adama
          </span>
          <div class="bg-red-50 border border-red-200 p-4 rounded-2xl rounded-tl-sm text-sm text-red-700 shadow-sm flex items-start gap-3">
            ...
            <div>
              <p class="font-semibold">AI is temporarily unavailable</p>
              <p class="text-xs mt-0.5 text-red-600 opacity-80">Please try again in a moment.</p>
            </div>
          </div>
        </div>
      </div>
...
// Auto-scroll AI sidebar to bottom on new messages or streaming updates
